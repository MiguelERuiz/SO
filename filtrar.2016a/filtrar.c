...
void recorrer_directorio(char* nombre_dir){
	...
	while((ent=readdir(dir))!=NULL)
	{
		...
		while((total=write(1,buff,read(fd,buff,4096)))){ // Está petando aquí!!!!!
			if(total<0){
...
void preparar_filtros(void)
{
	...
	for (i=n_filtros-1;i>=0;i--){	
		...
		pids[i]=p;//Guardamos el pid del proceso hijo
		switch(p){
...
void tratarAlarma(int n){
	...
	for(i=0; i<n_filtros;i++){
		activo = kill(pids[i],0); //Verificamos si el proceso i está activo
		if(activo<0){
			continue;
		}
		if(activo==0){
...
extern void preparar_alarma(){
	...
	if((valor=getenv("FILTRAR_TIMEOUT"))!=NULL){
		...
		int v;
                ...
                v = sigaction(SIGALRM,&act,NULL);
                if(v<0){
                        fprintf(stderr,"No se establece sigaction\n");
                        exit(1);
                }
		alarm(res);	
